...
/*   Updated: 2018/06/05 11:38:04 by ctrouill         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char				*jcolors(unsigned char owner, int bold);
...
int					valid_warriors(uint32_t i, char *argv[]);
...
int					load_instruction(t_arena *arena,
				t_process *process);
...
void				dump_cycle_memory(t_scene *scene,
							t_arena *arena);
int					player_ownership(unsigned int player, t_arena *arena);
void				init_values(size_t *i, int *xx, int *yy);
int					apply_pc(t_arena *arena, int i);
